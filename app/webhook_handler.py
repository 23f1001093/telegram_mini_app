...
from .TTS import TTSClient  # <-- gTTS version!

stt_client = STTClient()
llm_client = LLMClient()
tts_client = TTSClient()  # Use lang="en" or lang="hi" for Hindi if needed
...
async def webhook(request: Request):
    ...
    # 3. Transcribe speech to text with Vosk (offline)
    transcript = stt_client.transcribe_audio(audio_bytes)

    # 4. Get response from your LLM
    llm_response = llm_client.generate_response(transcript)

    # 5. Convert LLM text response to speech with gTTS
    audio_response = tts_client.synthesize_speech(llm_response)

    # 6. Send audio response as a Telegram voice message
